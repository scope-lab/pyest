...
def cislunar_example():
    ...
    # outputs x_f, STM, STT
    flow_info = lambda x,y,z,vx,vy,vz: integrator.dynVar_int2(
        [0, transfer_time], [x,y,z,vx,vy,vz], rtol=int_tol, atol=int_tol, output="final"
    )
    # outputs just the hessian
    hessian_func = lambda x,y,z,vx,vy,vz: integrator.dynVar_int2(
        [0, transfer_time], [x,y,z,vx,vy,vz], rtol=int_tol, atol=int_tol, output="final"
    )[2]
    # outputs just the jacobian
    jacobian_func = lambda x,y,z,vx,vy,vz: integrator.dynVar_int(
        [0, transfer_time], [x,y,z,vx,vy,vz],rtol=int_tol, atol=int_tol, output="final"
    )[1]
    # outputs flow of state only
    propagation = lambda x_0: integrator.dyn_int([0, transfer_time], x_0,
                                                  max_step=max_integrator_step,
                                                  t_eval=[transfer_time]).y[:,-1]

    # apply splitting methods
    split_opts = pygm.GaussSplitOptions(L=3, lam=1e-3, recurse_depth=3, min_weight=-np.inf)
    ...
    idx_pairs = [(0,1), (0,2), (1,2), (3,4), (4,5), (3,5), (0,4), (1,3)]
    axis_labels = ['$x$', '$y$', '$z$', r'$\dot{x}$', r'$\dot{y}$', r'$\dot{z}$']
    ...
    for idx_pair in idx_pairs:
        plt.figure()
        plt.scatter(np.array(final_samples)[:,idx_pair[0]], np.array(final_samples)[:,idx_pair[1]], marker='+', alpha=0.025)
        plt.xlabel(axis_labels[idx_pair[0]])
        plt.ylabel(axis_labels[idx_pair[1]])
        save_figure(example, "truth_scatter" + '_' + str(idx_pair[0]) + '_' + str(idx_pair[1]), plt.gca(), plt.gcf())
        plt.figure()
        plt.hist2d(np.array(final_samples)[:,idx_pair[0]], np.array(final_samples)[:,idx_pair[1]], 40)
        plt.xlabel(axis_labels[idx_pair[0]])
        plt.ylabel(axis_labels[idx_pair[1]])
        # save axis limits for later
        xlim[idx_pair] = plt.gca().get_xlim()
        ylim[idx_pair] = plt.gca().get_ylim()
        save_figure(example, "truth_hist" + '_' + str(idx_pair[0]) + '_' + str(idx_pair[1]), plt.gca(), plt.gcf())
