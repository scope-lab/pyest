...
def plot_split_and_transformed(p_split, py, split_method_str, example, dims=(0, 1),
    ...
    scatter_plt_overlay_args = {'s': 5**2, 'marker': 'x', 'zorder' : 2.1, 'color': 'w', 'alpha':0.9, 'linewidth':1}
    ...
    if scatter_means:
        ...
        plt.scatter(p_split.m[:,dims[0]], p_split.m[:,dims[1]],**scatter_plt_overlay_args)
    plt.grid()
    plt.xlabel('$x$')
    plt.ylabel('$y$')

    save_figure(example, split_method_str + "before_map" + str(dims), plt.gca(), plt.gcf())
    ...
    save_figure(example, split_method_str + '_' + str(dims[0]) + '_' + str(dims[1]), plt.gca(), plt.gcf())
...
def cartesian_to_polar_example():
    ...
    cartesian_to_polar = lambda x: [np.sqrt(x[0]**2 + x[1]**2), np.arctan2(x[1], x[0])]
    # Define the transformation from Polar to Cartesian coordinates
    polar_to_cartesian = lambda y: [y[0]*np.cos(y[1]), y[0]*np.sin(y[1])]
    ny = 2 # dimension of y
    nx = 2 # dimension of x
    ...
    py_true = lambda y: 0 if y[0]<0 else y[0]*p0(polar_to_cartesian(y))
    ...
    mean_polar, covariance_polar, Dt, sigmas, my = unscented_transform(p0.m[0], p0.P[0], cartesian_to_polar, sigma_pt_opts=sigma_pt_opts)

    # Create a Gaussian mixture for the transformed density
    p0_polar = GaussianMixture(weights, mean_polar, covariance_polar, cov_type='full')
    ...
    _, XX_true, YY_true = p0_polar.pdf_2d() # for laziness, use the linear transformation to generate grid points
    pp_true = np.array([py_true([x,y]) for x,y in zip(XX_true.ravel(), YY_true.ravel())]).reshape(XX_true.shape)
    fig, ax = plt.subplots()
    ...
    hessian = [sp.hessian(cartesian_to_polar_sym[i], [x, y]) for i in range(nx)]
    ...
    split_opts = pygm.GaussSplitOptions(L=3, lam=1e-3, recurse_depth=2, min_weight=1e-5)
    ...
    for split_method, args in recursive_split_args.items():
        ...
        py = transform_density_ekf(p_split, ny, cartesian_to_polar, jacobian_func )
        _, XX, YY = plot_split_and_transformed(p_split, py, split_method, example, xf_lim=x_lim, yf_lim=y_lim)
